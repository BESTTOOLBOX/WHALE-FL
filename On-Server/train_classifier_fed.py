...
def runExperiment():
    ...
    for epoch in range(last_epoch, cfg['num_epochs']['global'] + 1):
        ...
        save(save_result, './output/model/{}_checkpoint.csv'.format(cfg['model_tag']))
        if cfg['pivot'] < logger.mean['test/{}'.format(cfg['pivot_metric'])]:
            cfg['pivot'] = logger.mean['test/{}'.format(cfg['pivot_metric'])]
            shutil.copy('./output/model/{}_checkpoint.csv'.format(cfg['model_tag']),
                        './output/model/{}_best.csv'.format(cfg['model_tag']))
        logger.reset()
    logger.safe(False)
    return

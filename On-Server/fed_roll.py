...
class Federation:
    def __init__(self, global_parameters, rate, label_split, epoch):
                    ...
                    if 'weight' in parameter_type or 'bias' in parameter_type:
                        if parameter_type == 'weight':
                            if v.dim() > 1:
                                ...
                                if idx_i[m] is None:
                                    idx_i[m] = idx_i[m] = torch.arange(input_size, device=v.device)
                                input_idx_i_m = idx_i[m]
                                if k == output_weight_name:
                                    output_idx_i_m = idx_i[m] = torch.arange(input_size, device=v.device)
                                else:
                                    scaler_rate = self.model_rate[user_idx[m]] / cfg['global_model_rate']
                                    local_output_size = int(np.ceil(output_size * scaler_rate))
                                    output_idx_i_m = torch.roll(torch.arange(output_size, device=v.device), roll, -1)[:local_output_size]
                        ...
                        if 'weight' in parameter_type:
                            if v.dim() > 1:
                                ...
                                if 'embedding' in k.split('.')[-2]:
                                    ...
                                    input_idx_i_m = torch.roll(torch.arange(output_size, device=v.device), roll, -1)[:local_output_size]
                                    ...
                                    local_output_size = int(np.ceil(output_size // cfg['transformer']['num_heads']
                                                                    * scaler_rate))
                                    output_idx_i_m = (torch.roll(torch.arange(output_size, device=v.device), roll, -1).reshape(
